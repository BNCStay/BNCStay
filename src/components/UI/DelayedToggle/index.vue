...
import { defineComponent, onBeforeUnmount, PropType } from "@vue/runtime-core";
import {
  componentName,
...
export default defineComponent({
  ...
  setup(_props, { emit, slots, attrs }) {
    ...
    const noVModel = computed(
      () => typeof props.value.modelValue === "undefined"
    );
    ...
    const getDelay = computed(() => {
      ...
      if (typeof props.value.delay == "object") {
        value.on = convertToMilliSecond(
          `${props.value.delay.on || props.value.delay.enter || 0}`
        );
        value.off = convertToMilliSecond(
          `${props.value.delay.off || props.value.delay.leave || 0}`
        );
      }
    ...
    const nap = (on?: boolean) => {
      ...
      if (isOpenedAlready || isClosedAlready) {
        return;
      }
    ...
    const payload = computed<DefaultSlotProps>(() => ({
      active: active.value,
